...
//  File........: TC0161.c
//  Authors.....: Tom Tripp
//  Description.: This is a test case that tests Character Timeout Interrupt.
//
//  Conditions:
//    IFACE:  SPI or I2C
//    BAUD:    115200 bps
...
//  rreg( RBR );
  ...
  wreg( FCR, YES_FIFO | FCR_RX_TRIG_1B);
  ...
  wreg( LCR, lcr_out ); // Set Byte config
   ...
   __enable_interrupt();
   wreg( IER, IER_RX_DATA );

// Monitor SPI and /IRQ with LA and send two ASYNCH characters 
// from an external source. The interrupt will service the first
// character and the second character will cause a timeout
// interrupt which will be serviced and cleared.
